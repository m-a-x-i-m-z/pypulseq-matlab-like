...
def plot_nd(rec, complex=False, vmin=None, vmax=None, show=True):
    rec = np.squeeze(rec)
    ...
    if rec.ndim <= 2:
        ...
        if rec.shape[0] > 8:
            ...
            print(f'Only showing 8 slices out of {rec.shape[0]}')
        ...
        if rec.shape[0] > 8:
            print(f'Only showing 8 contrasts out of {rec.shape[0]}')
            rec = rec[np.floor(np.linspace(0,rec.shape[0]-1,8)).astype(int)]
            
        # Show up to 8 slices
        if rec.shape[1] > 8:
            print(f'Only showing 8 slices out of {rec.shape[1]}')
