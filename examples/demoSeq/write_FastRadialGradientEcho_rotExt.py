...
for i in range(1 - Ndummy, Nr + 1):
    ...
    seq.add_block(rf, gzComb, gxPre, rot)

    if i > 0:
        if gzSpoil is not None:
             seq.add_block(gx, adc, gzSpoil, rot)
        else:
             seq.add_block(gx, adc, rot)
    else:
        if gzSpoil is not None:
             seq.add_block(gx, gzSpoil, rot)
        else:
...
output_path = os.path.join(RESULTS_DIR, 'FastRadialGradientEcho_rotExt_py.seq')
