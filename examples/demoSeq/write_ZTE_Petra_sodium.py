...
def populate_subsequence(sys, seq, rf, adc, phi, theta, im_step, Ns, Ag, TR, Tt, FN, FO):
    ...
    if abs(Azc) > 1e-10:
        ...
        seq.add_block(delay_tr, dummy_grad)

    for j in range(1, im_step + 1):
        Glast = {'x': 0, 'y': 0, 'z': Azc}
...
system = Opts(max_grad=36, grad_unit='mT/m', max_slew=180, slew_unit='T/m/s',
              rf_ringdown_time=10e-6, rf_dead_time=100e-6, adc_dead_time=10e-6, gamma=11.262e6) # Sodium Gamma
...
seq.write(os.path.join(RESULTS_DIR, 'ZTE_Petra_sodium_py.seq'), remove_duplicates=False)
