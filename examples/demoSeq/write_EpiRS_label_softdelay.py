...
sat_ppm = -3.45
sat_freq = sat_ppm * 1e-6 * system.B0 * system.gamma  # Hz, used for bandwidth only
...
rf_fs.phase_ppm = -2 * np.pi * rf_fs.freq_ppm * rf_fs.center  # compensate for frequency-offset induced phase
...
min_TE = rf.shape_dur - calc_rf_center(rf)[0] + max(rf.ringdown_time, gz.fall_time) + calc_duration(gzReph) + \
    Nnav * calc_duration(gx) + calc_duration(gyPre) + \
...
for r in range(Nrep):
    ...
    for s in range(Nslices):
        ...
        if Nnav > 0:
            ...
            # softdelay TE 1st softdelay block.
            seq.add_block(make_delay(10e-6), make_soft_delay(numID=0, hint='TE', offset=-min_TE))
        ...
        # softdelay 2nd TE-related block
        seq.add_block(make_delay(max_TE - min_TE - 10e-6), make_soft_delay(hint='TE', factor=-1, offset=max_TE, numID=0))

        if s == 0 and r == 0:
            durPerSlc = np.sum(list(seq.block_durations.values()))

        # softdelay for setting the TR
        seq.add_block(make_delay(TRdelay_perSlice), make_soft_delay(numID=1, hint='TR', offset=-durPerSlc, factor=Nslices))

    seq.add_block(make_label(label='REP', type='INC', value=1))

# Workaround for PyPulseq core bug: soft_delay_library has numpy_data=True, converting floats to strings
for key, val in seq.soft_delay_library.data.items():
    seq.soft_delay_library.data[key] = (int(val[0]), float(val[1]), float(val[2]), str(val[3]))
...
output_path = os.path.join(RESULTS_DIR, 'EpiRS_label_softdelay_py.seq')
