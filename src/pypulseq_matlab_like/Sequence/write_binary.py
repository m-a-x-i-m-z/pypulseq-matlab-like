...
def write_binary(self, filename: str, create_signature: bool = True) -> Union[str, None]:
    if self.rotation_library.data:
        ...
        if 'ROTATIONS' not in required.split():
            self.set_definition('RequiredExtensions', f'{required} ROTATIONS'.strip())
    ...
    with open(filename, 'wb') as fid:
        ...
        if self.rf_shim_library.data:
            _write_int64(fid, codes['section']['rfshims'])
            _write_int32(fid, self.get_extension_type_ID('RF_SHIMS'))
            _write_int64(fid, len(self.rf_shim_library.data))
            for k, data in self.rf_shim_library.data.items():
                data = np.asarray(data, dtype=float).reshape(-1)
                _write_int32(fid, k)
                _write_int32(fid, len(data) // 2)
                fid.write(data.astype(np.float64).tobytes())

        if self.rotation_library.data:
            _write_int64(fid, codes['section']['rotations'])
            _write_int32(fid, self.get_extension_type_ID('ROTATIONS'))
            _write_int64(fid, len(self.rotation_library.data))
