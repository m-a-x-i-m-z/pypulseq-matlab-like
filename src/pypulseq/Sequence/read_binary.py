...
def read_binary(self, filename: str) -> None:
    ...
    with open(filename, 'rb') as fid:
        ...
        self.rf_shim_library = EventLibrary(numpy_data=True)
        ...
        while True:
            ...
            elif section == codes['section']['rfshims']:
                self.set_extension_string_ID('RF_SHIMS', int(_read_scalar(fid, np.int32)))
                _read_rf_shims(fid, self.rf_shim_library)
...
def _read_rf_shims(fid, library):
    num_events = int(_read_scalar(fid, np.int64))
    for _ in range(num_events):
        event_id = int(_read_scalar(fid, np.int32))
        num_chan = int(_read_scalar(fid, np.int32))
        data = np.frombuffer(fid.read(8 * 2 * num_chan), dtype=np.float64).copy()
        library.insert(event_id, data)


def _read_rotations(fid, library):
    num_events = int(_read_scalar(fid, np.int64))
    for _ in range(num_events):
        event_id = int(_read_scalar(fid, np.int32))
        quat = np.frombuffer(fid.read(8 * 4), dtype=np.float64).copy()
        norm = np.linalg.norm(quat)
        if norm > 0:
            quat = quat / norm
        library.insert(event_id, quat)
