...
def test_default_duration_warning():
    with pytest.raises(ValueError, match=r'Either bandwidth or duration must be defined.'):
...
def test_generation_methods():
    ...
    case3 = make_block_pulse(flip_angle=np.pi/3, bandwidth=1e3)
    assert isinstance(case3, SimpleNamespace)
    assert case3.shape_dur == 1 / (4 * 1e3)

    case4 = make_block_pulse(flip_angle=np.pi/3, bandwidth=1e3, time_bw_product=5)
